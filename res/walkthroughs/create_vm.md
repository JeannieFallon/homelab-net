...
Create a virtual machine Proxmox using Debian server ISO.
...
3. **Create the VM**
   ...
   - Use the following options to configure the VM:
       - **General**
         ...
         - Name: `[NAME]-[OS]-[VERSION]`
             - Suggested naming convention. This will be the Proxmox label for the VM, not the hostname.
       ...
       - **OS**
         ...
         - Version: `6.x - 2.6 Kernel` (safe default for Debian Bookworm)
       - **System**
...
4. **Start VM and install Debian**
    - In the Proxmox web UI, select the new VM
    ...
    - The following installation options are recommended for a lightweight, CLI-based Debian server environment:
       ...
       - **Configure the network**
          - Hostname: `[HOSTNAME]`
          - Domain name: `local`
              - **Note**: this gives the VM an FQDN of `[HOSTNAME].local`, which improves internal DNS or mDNS
       ...
       - **Software Selection**
         - Uncheck "Debian Desktop Environment" and "GNOME" options for efficient, headless server
         - Check "SSH server"
         - Check "standard system utilities"
    - When prompted, reboot the VM to complete installation.

4. **Configure VM**
    - In the Proxmox web UI, navigate to the *Console* tab and log in with the root user
    - Confirm WAN connectivity by pinging a Google DNS server:
      ```bash
      ping 8.8.8.8
      ```
    - Update packages and install minimum system utilites:
      ```bash
...
4. **Snapshot**
    ...
    - This VM should now be ready for automated configuration via Ansible playbooks.
